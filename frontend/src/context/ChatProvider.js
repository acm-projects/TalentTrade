...
const ChatProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if ((window.location.pathname !== "/signin" || window.location.pathname !== "/profile" || window.location.pathname !== "/profile") && !userInfo) { // && protectedRoutes.includes(window.location.pathname){
            navigate('/');
        }
    }, [navigate]);
